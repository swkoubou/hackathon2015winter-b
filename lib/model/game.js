    ...
    game.checkWrap(userId, blockQueries, function (err, canWrap) {
        ...
        for (y = 0; y < game.stageHeight; y++) {
            for (x = newBlock2d[y].length; x < game.stageWidth; x++) {
                newBlock = Block.createRandomNormalParams(game.blockLineTypeNum);
        ...
        Array.prototype.push.apply(params, _.flatten(newBlock2d));
        user.blocks.splice.apply(user.blocks, params);
...
Games.method('checkWrap', function (userId, blockQueries, callback) {
    ...
    var isValid = _.range(0, 1).some(function (cubeDirection) {
        ...
        return blockQueries.every(function (query, queryIndex, queries) {
            ...
            if (idx1 >= blocks.length || idx2 >= blocks.length) {
                return false;
            }

            var lineTypeIndex1 = query1.lineTypeIndex;
            var lineTypeIndex2 = query2.lineTypeIndex;
            var horizontal = lineTypeIndex1 ? vertical : -vertical;

            var lineType1 = blocks[idx1].lineTypes[lineTypeIndex1];
            var lineType2 = blocks[idx2].lineTypes[lineTypeIndex2];
            if (!lineType1 || !lineType2 || lineType1 !== lineType2) {
                return false;
            }
            ...
            var valids = [
                {   // straight
                    ...
                    y: query1.y + vertical,
                    lineTypeIndex: query1.direction
                },
                {   // reverse horizontal
                    x: query1.x,
                    y: query1.y + vertical,
                    lineTypeIndex: query1.lineTypeIndex ? 0 : 1
                },
                {   // reverse vertical
                    x: query1.x + horizontal,
                    y: query1.y,
            ...
            var isValid = valids.some(function (valid) {
                return valid.x === query2.x &&
                       valid.y === query2.y &&
                       valid.lineTypeIndex === query2.lineTypeIndex;
            });
